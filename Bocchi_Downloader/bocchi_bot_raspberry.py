...
# Предварительная сборка не прошедшая финальное тестирование.
...
def is_token_valid(context: ContextTypes.DEFAULT_TYPE) -> bool:
    """Проверяет наличие токена и его возраст (не более TOKEN_LIFETIME)."""
...
async def send_animated_message(bot, chat_id, text, delay=0.4, max_retries=3, **kwargs):
    """
    Отправляет сообщение с плавной анимацией появления (черновик → реальное сообщение).
    При ошибках выполняет повторные попытки. Если все попытки не удались — отправляет обычное сообщение.
    """
...
# --- ВОРКЕР С ПОВТОРНЫМИ ПОПЫТКАМИ ЗАГРУЗКИ ---
...
async def save_token(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Проверка, что токен действительно работает
...
async def process_accumulated_links(user_id, chat_id, context, token):
    ...
    try:
        client = await asyncio.to_thread(Client(token).init)
        # Проверка валидности токена перед обработкой
        await asyncio.to_thread(client.account_status)
    except Exception as e:
        logger.error(f"Не удалось инициализировать клиент или токен недействителен: {e}")
        await context.bot.send_message(chat_id=chat_id, text="❌ Ошибка авторизации или токен истёк. Попробуй обновить токен через меню.")
        return
    ...
    for url in raw_links:
        try:
            ...
            if "401" in error_str or "token" in error_str or "unauthorized" in error_str:
                logger.error(f"Токен истёк во время обработки: {e}")
                await context.bot.send_message(chat_id, "❌ Токен истёк во время обработки. Пожалуйста, обновите токен и попробуйте снова.")
                return
            else:
    ...
    if worker_busy:
        msg_text += f"\n🎸 Сейчас я немного занята, но скоро начну твои треки."
...
# --- СТАТУС С АНИМАЦИЕЙ И RETRY ДЛЯ ОБНОВЛЕНИЙ ---
...
# --- ПЕРИОДИЧЕСКАЯ ПРОВЕРКА ТОКЕНОВ (по времени жизни) ---
...
def main():
    ...
    if not shutil.which(DOWNLOADER_PATH):
        logger.error(f"❌ Загрузчик '{DOWNLOADER_PATH}' не найден в PATH. Установите yandex-music-downloader.")
        print(f"❌ Загрузчик '{DOWNLOADER_PATH}' не найден. Бот не может работать.")
        return
