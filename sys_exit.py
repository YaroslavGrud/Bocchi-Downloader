...
# и оставлен для попытки решить
# sys.exit которая ссылается на внешний загрузчик
...
async def handle_download(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Попытка извлечь ссылку из HTML-кода (например, из iframe)
    if 'iframe' in text and 'music.yandex' in text:
        ...
        if src_match:
            ...
            text = src_url  # Заменяем текст сообщения на извлеченную ссылку
    ...
    for u in urls:
        ...
        if parsed[0] is not None:
            if parsed[0] in ('iframe_playlist', 'uuid_playlist'):
                await context.bot.send_message(
                    chat_id=chat_id,
                    text=(
                        "⚠️ **Этот плейлист не обрабатывается напрямую.**\n\n"
                        "**Чтобы скачать его, сделайте следующее:**\n"
                        "1. Откройте плейлист в **браузере** (не в приложении).\n"
                        "2. Нажмите кнопку **«Поделиться»** (или три точки).\n"
                        "3. Выберите **«HTML-код»**.\n"
                        "4. Скопируйте весь полученный код и отправьте его мне.\n\n"
                        "Я извлеку рабочую ссылку и скачаю плейлист."
                    ),
                    parse_mode='Markdown',
                    reply_to_message_id=message.message_id
                )
...
async def process_accumulated_links(user_id, chat_id, context, token):
    ...
    for url in raw_links:
        ...
        try:
            if content_type == 'track':
                ...
                album_artist = ', '.join([a.get('name', '') for a in album_artists]) if album_artists else 'Разные исполнители'
    ...
    # Определяем тип пачки (если все ссылки одного типа, используем его)
